...
.dvl-mfrvp-edge.is-active::after{opacity:1;transform:scaleX(1.15);}`;(document.head||document.documentElement).appendChild(s);}catch(_){}})();
  ...
  function computeProfile(candles,requestedRows,vaPct,sym){
    ...
    for(const c of candles){
      ...
      // buy>0 (não >=0): alguns paths do app gravam buyVolume:0 quando o dado do
      // taker-buy não veio, e buy=0 fazia sell=v → delta 100% vermelho. Tratamos
      // buy<=0 como AUSENTE (cai no estimador por posição do fechamento), igual
      // ao Fixed Range VP normal (0095), que funciona.
      if(!(buy>0)){buy=NaN;}

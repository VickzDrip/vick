(function(){
  ...
  function draw(ctx, cfg){
    try{
      ...
      const cs = candles(); // Beta 1.642 — INTRABAR: a média acompanha o candle em formação (ao vivo). O flicker/"samba" foi resolvido na raiz (trava de DPR por gesto), não congelando a linha.
      if(!cs.length) return;

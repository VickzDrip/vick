const DVL_APP_VERSION = String(window.DVL_APP_VERSION || "Beta 1.508");
...
/* Beta 1.210 — Heikin conversion used to clone every visible candle on every
...
function dvlLowerPanelOn(){
  /* Beta 1.234 — deriva a visibilidade do painel inferior da MESMA lista que
...
function dvlPricePanelBottom(totalH){
  // Beta 1.310 — com osciladores, a timeline mora na base do canvas e não
...
/* Beta 1.324 — timeline física permanente.
...
function dvlSyncBottomTimeline1314(){
  try{
    ...
    ctx.font="10px system-ui";ctx.fillStyle="rgba(180,190,186,.86)";ctx.textBaseline="middle";
...
/* Beta 1.210 — fractional pan often changes only leftEdgeIndex, while the
...
function zoomChartAt(clientX, factor){
  ...
  /* Beta 1.179 desktop mouse fix: the previous wheel path called
  ...
  try{
    ...
    if(Array.isArray(all)){
      ...
      if(assetDropdownOpen && els.assetDropdown && els.assetDropdown.classList.contains("is-open")){
        ...
        /* Beta 1.206: closed menus stay frozen. Rebuilding their entire DOM on
...
/* Beta 1.383 — Fix da SELEÇÃO de 15s/30s. O sistema de timeframe tem módulos
...
async function fetchKlinesSmart(sym, iv, targetLimit){
  ...
  if(parsed.unit === "s"){
    // Beta 1.383 — 15s/30s: tenta PRIMEIRO o recorder do próprio servidor DVL
...
function resizeCanvas(){
  ...
  /* Beta 1.197 — dynamic interaction resolution. A retina canvas at DPR 2.5
  ...
  /* Beta 1.209 — interaction quality tier. While pan/zoom is active the frame
  ...
  if(ctx && !ctx.__dvlGlowHooked){
    /* Beta 1.188 — shadowBlur (glow) é o op MAIS caro do canvas: recalcula uma
...
function drawSoon(){
  /* Beta 1.181 — keep exactly one pending paint. High polling-rate mice can
     ...
     Beta 1.249 — scheduler ÚNICO: um só requestAnimationFrame por frame, com
...
/* ── Beta 1.249 — Ultra Low Latency: instrumentação + gap adaptativo ──────────
...
/* Beta 1.209 — layout DOM synchronization is NOT part of every canvas frame.
...
function draw(){
  ...
  /* Beta 1.209 — the next operation paints an opaque full-canvas background,
  ...
  /* Beta 1.321 — a timeline não usa mais camada DOM externa. Ela será
  ...
  /* Beta 1.324 — a escala final mora num canvas permanente dentro do
...
/* Beta 1.210 — cache percentile autoscale for the same integer candle
...
function __dvlAutoScalePercentiles1210(drawView,win){
  ...
  /* Beta 1.211 — desktop wheel zoom is now a real interaction cycle. Keep the
...
/* Beta 1.235 — ESTABILIZADOR GLOBAL DA ESCALA DE PREÇO.
...
if(window.DVLSpikeZonesDraw){
  ...
  // Beta 1.245 — usa o close da vela em formação (atualizado por WS/@aggTrade em
  ...
  /* Beta 1.189 — batch dos pavios (wicks). Antes cada vela dava um ctx.stroke()
  ...
  /* Beta 1.189 — desenha todos os pavios agrupados por cor (2 strokes no lugar
  ...
  /* Beta 1.190 — triângulos de COMPRA/VENDA dos "vzinhos" do RSI de exaustão.
...
function setupChartInteractions(){
  ...
  /* Beta 1.187 — cache do retângulo do canvas p/ matar o layout-thrash.
  ...
  /* Beta 1.188 — marca "interagindo" p/ o interceptor de glow desligar o
  ...
  function _onDown(ev){
    ...
    /* Beta 1.184 — divider owns this gesture BEFORE chart pan/crosshair.
    ...
    /* Beta 1.227 — não iniciar pan quando o toque começa numa linha do Risk Engine */
...
/* DVL Beta 1.204 — live-data render gate.
...
let __DVL_LIVE_RENDER_GAP_1204 = 33;   // Beta 1.249 — ADAPTATIVO (16–60ms): ajustado pelo custo real do render
...
function requestLiveChartRender(force){
  /* Beta 1.387 — CAMINHO RÁPIDO tick-a-tick (spec §10). Antes o live passava por
...
/* DVL Beta 1.206 — feed lifecycle instead of permanent full reloads.
...
/* Beta 1.203 — removed the unconditional one-second full redraw.
...
function _glueFormingCandle(){
  if(window.__DVL_REPLAY_ACTIVE) return;
  if(!klines.length) return;
  /* Beta 1.386 — quando há feed de kline ao vivo (Binance), a vela já vem do
...
/* Beta 1.388 — watchdog de "socket morto silencioso". Os WS da Binance às vezes
...
/* Beta 1.389 — aggTrade "vivo" = recebeu negócio nos últimos 5s (agora ele vem
...
function _klWsConnect(){
  ...
  try{
    /* Beta 1.389 — CONEXÃO COMBINADA kline+aggTrade (spot). O @aggTrade numa
       ...
       Beta 1.384: spot, não futuros (fstream é geo-bloqueado no BR). */
...
// ── Beta 1.245 — Live price por TRADE (@aggTrade): fluidez em TF baixo ──────
...
/* Beta 1.389 — o @aggTrade agora vem na CONEXÃO COMBINADA do _klWs (e do _nnWs
...
function _nnWsConnect(){
  ...
  try{
    /* Beta 1.389 — combinado kline_base + aggTrade (mesmo motivo do _klWs): dá
...
/* Beta 1.388 — WATCHDOG de socket mudo. Se um WS está OPEN mas parou de mandar

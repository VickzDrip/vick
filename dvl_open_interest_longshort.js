/* ═══ DVL_BETA_0325_COINGLASS_OSCILLATORS_SCRIPT (L38770-38839) ═══ */
(function(){
'use strict';
...
/* ═══ DVL_BETA_0328_CG_CARD_RESIZE_CONTROLS_SCRIPT (L38896-39040) ═══ */
(function(){
'use strict';
...
/* ═══ DVL_BETA_0330_CG_MODAL_RESET_SCRIPT (L39104-39309) ═══ */
(function(){
  'use strict';
...
/* ═══ DVL_BETA_0331_CG_GEAR_TAP_SCRIPT (L39330-39435) ═══ */
(function(){
  'use strict';
  const VER='Beta 0.331';
  let lastOpen=0;
  let raf=0;
  function q(s,r){return (r||document).querySelector(s);}
  function qa(s,r){return Array.from((r||document).querySelectorAll(s));}
  ...
  function forceVersion(){
    ...
    try{
      window.DVL_CHANGELOG=window.DVL_CHANGELOG||[];
      [
        'CoinGlass gear opens on simple tap/click, no long press required.',
        'CG Taker Long/Short settings gained a Média toggle to disable the cyan average/ratio line.'
      ].forEach(note=>{if(!window.DVL_CHANGELOG.some(x=>x&&x.version===VER&&x.note===note))window.DVL_CHANGELOG.unshift({version:VER,date:new Date().toISOString(),note});});
    }catch(_){ }
  }
  function storeKey(id){return 'dvl_cg_setting_'+id;}
  function getVal(id,def){try{const v=localStorage.getItem(storeKey(id));if(v!==null&&v!==undefined&&v!=='')return v;}catch(_){}const el=q('#'+id);return el&&el.value!==undefined&&el.value!==''?el.value:def;}
  function setVal(id,v){
    try{localStorage.setItem(storeKey(id),String(v));}catch(_){}
    let el=q('#'+id);
    if(!el){el=document.createElement('input');el.type='hidden';el.id=id;el.dataset.dvl331Store='1';document.body.appendChild(el);}
    try{el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}catch(_){}
    try{if(typeof window.drawSoon==='function')window.drawSoon();else if(typeof window.draw==='function')window.draw();}catch(_){}
  }
  function gearKey(target){
    if(!target||!target.closest)return null;
    const cardGear=target.closest('.gear[data-settings="cgOpenInterest"],.gear[data-settings="cgLongShort"]');
    if(cardGear)return cardGear.getAttribute('data-settings');
    const oi=target.closest('#dvl325_cgOi_gear,#dvl328_cgOi_gear,.dvl325-cg-gear-overlay[data-kind="cgOi"]');
    if(oi)return 'cgOpenInterest';
    const ls=target.closest('#dvl325_cgLs_gear,#dvl328_cgLs_gear,.dvl325-cg-gear-overlay[data-kind="cgLs"]');
    if(ls)return 'cgLongShort';
  ...
  function nativeOpen(key){
    // Prefer the stable 0.330 modal path by clicking its existing target if present.
    let el=null;
    if(key==='cgOpenInterest') el=q('.gear[data-settings="cgOpenInterest"]')||q('#dvl325_cgOi_gear')||q('#dvl328_cgOi_gear');
    else el=q('.gear[data-settings="cgLongShort"]')||q('#dvl325_cgLs_gear')||q('#dvl328_cgLs_gear');
    if(el){
      try{ el.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window})); return true; }catch(_){ }
      try{ clickCenter(el).click(); return true; }catch(_){ }
    }
    if(typeof window.openInputModal==='function'){
      try{ window.openInputModal(key,el||null); return true; }catch(_){ }
    }
    return false;
  }
  function openNow(key){
    const now=Date.now();
    if(now-lastOpen<260)return;
    lastOpen=now;
    nativeOpen(key);
    setTimeout(enhanceModal,0);
    setTimeout(enhanceModal,90);
    setTimeout(enhanceModal,240);
  }
  function bindFastGear(){
    if(document.documentElement.dataset.dvl331FastGear==='1')return;
    document.documentElement.dataset.dvl331FastGear='1';
    ['pointerdown','touchstart','mousedown','click'].forEach(evt=>{
      document.addEventListener(evt,function(e){
        const key=gearKey(e.target);
        if(!key)return;
        stop(e);
        openNow(key);
      },{capture:true,passive:false});
  ...
  function enhanceModal(){
    const body=q('#inputModalBody');
    if(!body||!body.dataset.dvl330Cg)return;
    const isLs=(q('#inputModalTitle')&&/Taker|L\/S|Long/i.test(q('#inputModalTitle').textContent||''))||q('[data-cg-src="cgLsMode"]',body);
    if(!isLs)return;
    if(q('[data-cg-src="cgLsShowAverage"]',body))return;
    const row=document.createElement('div');
    row.className='dvl330-cg-row dvl331-avg-row';
    row.innerHTML='<span class="k">Média</span><select class="select" data-dvl330-ui="1" data-cg-src="cgLsShowAverage"><option value="on">Ligada</option><option value="off">Desligada</option></select>';
    const modeRow=q('[data-cg-src="cgLsMode"]',body)?.closest('.dvl330-cg-row');
    if(modeRow&&modeRow.parentNode)modeRow.parentNode.insertBefore(row,modeRow.nextSibling);else body.appendChild(row);
    const sel=q('[data-cg-src="cgLsShowAverage"]',row);
    sel.value=getVal('cgLsShowAverage','on');
    sel.addEventListener('input',()=>setVal('cgLsShowAverage',sel.value),{passive:true});
    sel.addEventListener('change',()=>setVal('cgLsShowAverage',sel.value),{passive:true});
  }
  function boot(){raf=0;forceVersion();bindFastGear();enhanceModal();}
  function schedule(){if(raf)return;raf=requestAnimationFrame(boot);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  [80,220,600,1400,3000].forEach(ms=>setTimeout(schedule,ms));
...
/* ═══ DVL_BETA_0332_CG_GEAR_CLICK_FIXED_SCRIPT (L39506-39620) ═══ */
(function(){
  'use strict';
...
/* ═══ DVL_BETA_0326_COINGLASS_POLISH_SCRIPT (L57117-57207) ═══ */
(function(){
  const VER='Beta 0.326';
  let raf=0;
  function q(s,r){return (r||document).querySelector(s);}
  function qa(s,r){return Array.from((r||document).querySelectorAll(s));}
  function forceVersion(){
    ...
    try{
      window.DVL_CHANGELOG=window.DVL_CHANGELOG||[];
      const note='CoinGlass oscillator visual polish: compact cards, hidden click-box overlays, shorter panel labels and native DVL layout alignment.';
      if(!window.DVL_CHANGELOG.some(x=>x&&x.version===VER&&x.note===note)) window.DVL_CHANGELOG.unshift({version:VER,date:new Date().toISOString(),note});
    }catch(_){ }
  }
  function tuneHeights(){
    try{
      const oiRaw=parseInt(localStorage.getItem('dvl_cgOpenInterestH')||'0',10);
      const lsRaw=parseInt(localStorage.getItem('dvl_cgLongShortH')||'0',10);
      if(!oiRaw || oiRaw===112){ window._cgOpenInterestH=86; localStorage.setItem('dvl_cgOpenInterestH','86'); }
      if(!lsRaw || lsRaw===104){ window._cgLongShortH=78; localStorage.setItem('dvl_cgLongShortH','78'); }
    }catch(_){ window._cgOpenInterestH=86; window._cgLongShortH=78; }
  }
  function polishCards(){
    const data=[
      ['cgOpenInterest','CG Open Interest','OI CANDLES'],
      ['cgLongShort','CG Long/Short','ACCOUNTS RATIO']
    ];
    data.forEach(([key,name,sub])=>{
      const card=q('[data-card="'+key+'"]');
      if(!card) return;
      card.classList.add('dvl326-cg-polished');
      const n=q('.name',card); if(n) n.textContent=name;
      const s=q('.ind-sub',card); if(s) s.textContent=sub;
      const ico=q('.dvl-icd-ico',card); if(ico) ico.textContent=key==='cgOpenInterest'?'OI':'L/S';
      const actions=q('.ind-actions',card);
      const row=q('.row[data-ind="'+key+'"]',card);
      const gear=q('.gear[data-settings="'+key+'"]',card);
      const star=q('.dvl-star[data-star="'+key+'"]',card);
      if(actions){
        if(row && row.parentNode!==actions) actions.appendChild(row);
        if(gear && gear.parentNode!==actions) actions.appendChild(gear);
        if(star && star.parentNode!==actions) actions.appendChild(star);
      }
    });
  }
  function hideOverlayPaint(){
    qa('#chartWrap .dvl325-cg-gear-overlay,#chartWrap .dvl325-cg-eye-overlay,#chartWrap button[id^="dvl325_cgOi_"],#chartWrap button[id^="dvl325_cgLs_"]').forEach(b=>{
  ...
  function wrapDrawText(){
    if(typeof window.draw!=='function' || window.draw.__dvl326CgText) return;
    const old=window.draw;
    const wrapped=function(){
      const canvas=q('#chart');
      const ctx=canvas&&canvas.getContext?canvas.getContext('2d'):null;
      if(!ctx) return old.apply(this,arguments);
      const original=ctx.fillText;
      ctx.fillText=function(text,x,y,maxWidth){
        let t=String(text==null?'':text);
        if(t==='CG AGG OI') t='OI AGG';
        if(t==='CG L/S ACCOUNTS') t='L/S ACCOUNTS';
        if(t==='BINANCE'||t==='BINANCE GLOBAL'||t==='BINANCE TOP'||t==='BINANCE FALLBACK') return;
        return original.call(this,t,x,y,maxWidth);
      };
      try{return old.apply(this,arguments);}finally{ctx.fillText=original;}
    };
    wrapped.__dvl326CgText=true;
  ...
  function schedule(){if(raf)return;raf=requestAnimationFrame(boot);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  [100,300,700,1400,2600,4200].forEach(ms=>setTimeout(schedule,ms));
  setInterval(function(){polishCards();hideOverlayPaint();},1600);
})();

/* ═══ DVL_BETA_0327_CG_NATIVE_OSC_SCRIPT (L57227-57292) ═══ */
(function(){
'use strict';
...
/* ═══ DVL_BETA_0329_CG_SETTINGS_RESET_SCRIPT (L57313-57514) ═══ */
(function(){
  'use strict';
  const VER='Beta 0.329';
  const CG_KEYS=['cgOpenInterest','cgLongShort'];
  let activeCgSettings=null;
  let keepTimer=0;
  let raf=0;
  ...
  function forceVersion(){
    try{window.DVL_APP_VERSION=VER;}catch(_){}
    qa('#dvlVersionBadge,.dvl-version-badge,.dvl-version-header,.dvl-version-logo-hidden,.beta-badge,.version-badge').forEach(el=>{
      if(el)el.textContent=(window.DVL_APP_VERSION||VER);
    });
    try{
      window.DVL_CHANGELOG=window.DVL_CHANGELOG||[];
      [
        'Fixed CoinGlass oscillator settings drawer so its content no longer disappears when cards refresh.',
        'Fit/scale buttons now reset oscillator scales and CoinGlass panel heights back to the initial layout.'
      ].forEach(note=>{
        if(!window.DVL_CHANGELOG.some(x=>x&&x.version===VER&&x.note===note)){
          window.DVL_CHANGELOG.unshift({version:VER,date:new Date().toISOString(),note});
        }
      });
    }catch(_){}
  }

  function cardFor(key){return q('.ind-card[data-card="'+key+'"]');}
  function gearFor(key){return q('.gear[data-settings="'+key+'"]',cardFor(key))||q('.gear[data-settings="'+key+'"]');}
  function body(){return q('#inputModalBody');}
  function modal(){return q('#inputModal');}
  function title(){return q('#inputModalTitle');}

  function bindCgInputs(){
    ['cgDataSource','cgApiKey','cgSharedPeriod','cgOiUnit','cgOpenInterestHeight','cgExchange','cgLsMode','cgLsShowAverage','cgLongShortHeight','cgTakerExchanges'].forEach(id=>{
      const el=q('#'+id);
      if(!el||el.dataset.dvl329Bound)return;
      el.dataset.dvl329Bound='1';
      const fire=function(){
        try{
          if(id==='cgOpenInterestHeight'&&window._cgOpenInterestH!==undefined){
            const n=parseInt(el.value||window._cgOpenInterestH,10);
            if(Number.isFinite(n)){window._cgOpenInterestH=Math.max(62,Math.min(240,n));localStorage.setItem('dvl_cgOpenInterestH',String(window._cgOpenInterestH));}
          }
          if(id==='cgLongShortHeight'&&window._cgLongShortH!==undefined){
            const n=parseInt(el.value||window._cgLongShortH,10);
            if(Number.isFinite(n)){window._cgLongShortH=Math.max(58,Math.min(230,n));localStorage.setItem('dvl_cgLongShortH',String(window._cgLongShortH));}
          }
        }catch(_){}
        try{if(typeof window.drawSoon==='function')window.drawSoon();else if(typeof window.draw==='function')window.draw();}catch(_){}
      };
  ...
  function keepCgSettingsInModal(){
    if(!activeCgSettings)return;
    const m=modal(), b=body();
    if(!m||!b||!m.classList.contains('show')){activeCgSettings=null;return;}
    const settings=q('#settings-'+activeCgSettings);
    if(!settings)return;
    if(settings.parentNode!==b)b.appendChild(settings);
    const t=title();
    if(t){
      t.textContent=(activeCgSettings==='cgOpenInterest'?'CG Open Interest':'CG Taker L/S')+' — INPUTS';
    }
    bindCgInputs();
  }
  ...
  function startKeepLoop(){
    ...
    keepTimer=setInterval(function(){
      if(!activeCgSettings){clearInterval(keepTimer);keepTimer=0;return;}
      keepCgSettingsInModal();
    },60);
  }

  function wrapModal(){
    if(window.openInputModal&& !window.openInputModal.__dvl329Wrapped){
      const oldOpen=window.openInputModal;
      window.openInputModal=function(key,gear){
        if(CG_KEYS.includes(key)){
          const realGear=gear||gearFor(key);
          activeCgSettings=key;
          const r=oldOpen.call(this,key,realGear||null);
          keepCgSettingsInModal();
          setTimeout(keepCgSettingsInModal,0);
          setTimeout(keepCgSettingsInModal,80);
          setTimeout(keepCgSettingsInModal,240);
          startKeepLoop();
          return r;
        }
        return oldOpen.apply(this,arguments);
      };
      window.openInputModal.__dvl329Wrapped=true;
    }
    if(window.closeInputModal && !window.closeInputModal.__dvl329Wrapped){
      const oldClose=window.closeInputModal;
      window.closeInputModal=function(){
        activeCgSettings=null;
        return oldClose.apply(this,arguments);
      };
      window.closeInputModal.__dvl329Wrapped=true;
    }
  }

  function bindGears(){
    CG_KEYS.forEach(key=>{
      qa('.gear[data-settings="'+key+'"]').forEach(g=>{
        if(g.dataset.dvl329GearBound)return;
        g.dataset.dvl329GearBound='1';
        g.addEventListener('pointerdown',function(e){stop(e); if(typeof window.openInputModal==='function')window.openInputModal(key,g);},{capture:true,passive:false});
        g.addEventListener('click',function(e){stop(e); if(typeof window.openInputModal==='function')window.openInputModal(key,g);},{capture:true,passive:false});
      });
    });
  }

  function setGlobalLexical(name,value){
    try{ (0,eval)(name+'='+JSON.stringify(value)); return true; }catch(_){}
    try{ eval(name+'='+JSON.stringify(value)); return true; }catch(_){}
    return false;
  }

  function resetOscillatorsToInitial(){
    try{
      if(window.S){
        S.oscScale=S.oscScale||{};
        S.oscScale.flow={center:0,range:200};
    ...
    /* Restore initial panel heights. Existing Flow/Clarity heights live in top-level
       lexical vars, so use global eval as a compatibility bridge. */
    setGlobalLexical('_hvnClarityH',166);
    setGlobalLexical('_flowAccelH',120);
    try{
      window._hvnClarityH=166;
      window._flowAccelH=120;
      window._cgOpenInterestH=112;
      window._cgLongShortH=104;
      localStorage.setItem('dvl_hvnClarityH','166');
      localStorage.setItem('dvl_flowAccelH','120');
      localStorage.setItem('dvl_cgOpenInterestH','112');
      localStorage.setItem('dvl_cgLongShortH','104');
      const oi=q('#cgOpenInterestHeight');if(oi)oi.value='112';
      const ls=q('#cgLongShortHeight');if(ls)ls.value='104';
    }catch(_){}

    try{if(typeof window.drawSoon==='function')window.drawSoon();else if(typeof window.draw==='function')window.draw();}catch(_){}
  }

  function wrapFitButtons(){
    if(document.documentElement.dataset.dvl329FitBound==='1')return;
    document.documentElement.dataset.dvl329FitBound='1';

    ['btnFit','btnLive'].forEach(id=>{
      const btn=q('#'+id);
      if(!btn)return;
      btn.addEventListener('click',function(){setTimeout(resetOscillatorsToInitial,0);},{capture:false,passive:true});
      btn.addEventListener('touchend',function(){setTimeout(resetOscillatorsToInitial,0);},{capture:false,passive:true});
      btn.addEventListener('pointerup',function(){setTimeout(resetOscillatorsToInitial,0);},{capture:false,passive:true});
    });

    try{
      if(typeof window.fitClose==='function'&&!window.fitClose.__dvl329Wrapped){
        const old=window.fitClose;
        window.fitClose=function(){const r=old.apply(this,arguments);resetOscillatorsToInitial();return r;};
        window.fitClose.__dvl329Wrapped=true;
      }
      if(typeof window.fitAll==='function'&&!window.fitAll.__dvl329Wrapped){
        const old=window.fitAll;
        window.fitAll=function(){const r=old.apply(this,arguments);resetOscillatorsToInitial();return r;};
        window.fitAll.__dvl329Wrapped=true;
      }
    }catch(_){}
  }
  ...
  function boot(){
    ...
    bindCgInputs();
    wrapFitButtons();
    if(activeCgSettings)keepCgSettingsInModal();
  }
  function schedule(){if(raf)return;raf=requestAnimationFrame(boot);}

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});
  else boot();
  [100,260,700,1400,2600,4200].forEach(ms=>setTimeout(schedule,ms));
  /* interval removed — Beta 0.334 modal supersedes 0.329 */
})();

/* ═══ DVL_BETA_0333_CG_GEAR_MODAL_FINAL_SCRIPT (L57556-57701) ═══ */
(function(){
  'use strict';
  const VER='Beta 0.333';
  const IDS=['cgDataSource','cgApiKey','cgSharedPeriod','cgOiUnit','cgOpenInterestHeight','cgExchange','cgLsMode','cgLsShowAverage','cgLongShortHeight','cgTakerExchanges'];
  let lastOpen=0;
  let raf=0;
  function q(s,r){return (r||document).querySelector(s);}
  ...
  function existing(id){return qa('#'+id).find(el=>!el.dataset.dvl333Ui)||null;}
  function ensureOriginal(id){
    let el=existing(id);
    if(!el){el=document.createElement('input');el.type='hidden';el.id=id;el.dataset.dvl333Store='1';document.body.appendChild(el);}
    return el;
  }
  function getVal(id){
    try{const v=localStorage.getItem(storageKey(id)); if(v!==null&&v!==undefined&&v!=='')return v;}catch(_){}
    const el=existing(id); if(el&&el.value!==undefined&&el.value!=='')return el.value;
    return defaults(id);
  }
  function setVal(id,v,redraw){
    const val=String(v==null?'':v);
    try{localStorage.setItem(storageKey(id),val);}catch(_){}
    const el=ensureOriginal(id);
    try{el.value=val;}catch(_){}
    if(id==='cgOpenInterestHeight'){
      const n=parseInt(val,10); if(Number.isFinite(n)){window._cgOpenInterestH=Math.max(78,Math.min(240,n));try{localStorage.setItem('dvl_cgOpenInterestH',String(window._cgOpenInterestH));}catch(_){} }
    }
    if(id==='cgLongShortHeight'){
      const n=parseInt(val,10); if(Number.isFinite(n)){window._cgLongShortH=Math.max(72,Math.min(230,n));try{localStorage.setItem('dvl_cgLongShortH',String(window._cgLongShortH));}catch(_){} }
    }
  ...
  function forceVersion(){
    ...
    try{
      window.DVL_CHANGELOG=window.DVL_CHANGELOG||[];
      const notes=[
        'CoinGlass gear opens with one normal tap from sidebar and chart controls; old touch handlers no longer swallow the click.',
        'Dedicated CoinGlass settings modal added with stable Taker average on/off control.'
      ];
      notes.forEach(note=>{if(!window.DVL_CHANGELOG.some(x=>x&&x.version===VER&&x.note===note))window.DVL_CHANGELOG.unshift({version:VER,date:new Date().toISOString(),note});});
    }catch(_){}
  }
  function select(id,opts){const v=getVal(id);return '<select data-dvl333-ui="1" data-cg-id="'+id+'">'+opts.map(o=>'<option value="'+esc(o[0])+'" '+(String(o[0])===String(v)?'selected':'')+'>'+esc(o[1])+'</option>').join('')+'</select>';}
  function input(id,type,ph){return '<input data-dvl333-ui="1" data-cg-id="'+id+'" type="'+type+'" value="'+esc(getVal(id))+'" placeholder="'+esc(ph||'')+'">';}
  function num(id,min,max){return '<input data-dvl333-ui="1" data-cg-id="'+id+'" type="number" min="'+min+'" max="'+max+'" value="'+esc(getVal(id))+'">';}
  function row(label,control){return '<div class="dvl333-cg-row"><span class="k">'+label+'</span>'+control+'</div>';}
  function bodyHtml(key){
    if(key==='cgOpenInterest'){
      return row('Fonte',select('cgDataSource',[['auto','Auto CG/Binance'],['coinglass','CoinGlass API'],['binance','Binance público']]))+
        ...
        row('Unidade',select('cgOiUnit',[['usd','USD'],['coin','Coin']]))+
        row('Altura',num('cgOpenInterestHeight',78,240))+
        '<div class="dvl333-cg-note">Com API key usa CoinGlass aggregated OI. Sem key, usa fallback público Binance.</div>';
    }
    return row('Fonte',select('cgDataSource',[['auto','Auto CG/Binance'],['coinglass','CoinGlass API'],['binance','Binance público']]))+
      row('Exchange',select('cgExchange',[['Binance','Binance'],['Bybit','Bybit'],['OKX','OKX'],['Bitget','Bitget']]))+
      row('Modo',select('cgLsMode',[['taker','Taker buy/sell'],['accounts-global','Accounts global'],['accounts-top','Top accounts']]))+
      ...
      row('CG Exch.',input('cgTakerExchanges','text','Binance,OKX,Bybit'))+
      row('Altura',num('cgLongShortHeight',72,230))+
      '<div class="dvl333-cg-note">Padrão correto: Taker buy/sell long-short ratio. A média pode ser desligada aqui.</div>';
  }
  function ensureModal(){
    let scr=q('#dvl333CgScrim'), mod=q('#dvl333CgModal');
    if(!scr){scr=document.createElement('div');scr.id='dvl333CgScrim';document.body.appendChild(scr);}
    if(!mod){
      mod=document.createElement('div');mod.id='dvl333CgModal';mod.innerHTML='<div class="dvl333-cg-head"><div class="dvl333-cg-title" id="dvl333CgTitle"></div><button type="button" class="dvl333-cg-x" id="dvl333CgClose">×</button></div><div class="dvl333-cg-body" id="dvl333CgBody"></div>';
      document.body.appendChild(mod);
    }
    if(!scr.dataset.dvl333Bound){scr.dataset.dvl333Bound='1';['pointerdown','touchstart','click'].forEach(ev=>scr.addEventListener(ev,e=>{stop(e);closeModal();},{capture:true,passive:false}));}
    const x=q('#dvl333CgClose');
    if(x&&!x.dataset.dvl333Bound){x.dataset.dvl333Bound='1';['pointerdown','touchstart','click'].forEach(ev=>x.addEventListener(ev,e=>{stop(e);closeModal();},{capture:true,passive:false}));}
    return {scr,mod};
  }
  function openModal(key){
    const now=Date.now(); if(now-lastOpen<130)return; lastOpen=now;
    IDS.forEach(id=>setVal(id,getVal(id),false));
    const {scr,mod}=ensureModal();
    const title=q('#dvl333CgTitle'), body=q('#dvl333CgBody');
    if(title)title.textContent=(key==='cgOpenInterest'?'CG Open Interest':'CG Taker L/S')+' — INPUTS';
  ...
  function closeModal(){const scr=q('#dvl333CgScrim'),mod=q('#dvl333CgModal');if(scr)scr.classList.remove('show');if(mod)mod.classList.remove('show');}
  function point(e){
    if(e.touches&&e.touches[0])return {x:e.touches[0].clientX,y:e.touches[0].clientY};
    if(e.changedTouches&&e.changedTouches[0])return {x:e.changedTouches[0].clientX,y:e.changedTouches[0].clientY};
    return {x:e.clientX,y:e.clientY};
  }
  function keyFromHitbox(e){
    try{
      const wrap=q('#chartWrap'); const hits=window._dvlCgOscHit||{}; if(!wrap||!hits)return null;
      const p=point(e); if(!Number.isFinite(p.x)||!Number.isFinite(p.y))return null;
      const r=wrap.getBoundingClientRect(); const x=p.x-r.left, y=p.y-r.top;
      const pairs=[['cgOi','cgOpenInterest'],['cgLs','cgLongShort']];
      for(const [k,key] of pairs){const b=hits[k]&&hits[k].gear;if(b&&x>=b.x1&&x<=b.x2&&y>=b.y1&&y<=b.y2)return key;}
    }catch(_){}
    return null;
  }
  function eyeFromHitbox(e){
    try{
      const wrap=q('#chartWrap'); const hits=window._dvlCgOscHit||{}; if(!wrap||!hits)return null;
      const p=point(e); if(!Number.isFinite(p.x)||!Number.isFinite(p.y))return null;
      const r=wrap.getBoundingClientRect(); const x=p.x-r.left, y=p.y-r.top;
      for(const k of ['cgOi','cgLs']){const b=hits[k]&&hits[k].eye;if(b&&x>=b.x1&&x<=b.x2&&y>=b.y1&&y<=b.y2)return k;}
    }catch(_){}
    return null;
  }
  function keyFromTarget(t){
    ...
    if(g)return g.getAttribute('data-settings');
    const id=String(t.id||'');
    if(id.toLowerCase().includes('cgoi')&&id.toLowerCase().includes('gear'))return 'cgOpenInterest';
    if(id.toLowerCase().includes('cgls')&&id.toLowerCase().includes('gear'))return 'cgLongShort';
    return null;
  }
  function bindCapture(){
    if(window.__dvl333CgCaptureBound)return; window.__dvl333CgCaptureBound=true;
    ['pointerdown','touchstart','mousedown','click'].forEach(evt=>{
      window.addEventListener(evt,function(e){
        const key=keyFromTarget(e.target)||keyFromHitbox(e);
        if(key){stop(e);openModal(key);return;}
        const eye=eyeFromHitbox(e);
        if(eye){stop(e);window['__dvl_'+eye+'_hidden']=!window['__dvl_'+eye+'_hidden'];try{localStorage.setItem('dvl_'+eye+'_hidden',window['__dvl_'+eye+'_hidden']?'1':'');}catch(_){}try{if(typeof window.drawSoon==='function')window.drawSoon();else if(typeof window.draw==='function')window.draw();}catch(_){} }
      },{capture:true,passive:false});
    });
    document.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal();},true);
  }
  function syncOrig(){IDS.forEach(id=>setVal(id,getVal(id),false));}
  function boot(){raf=0;forceVersion();ensureModal();syncOrig();bindCapture();}
  function schedule(){if(raf)return;raf=requestAnimationFrame(boot);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  [60,180,420,900,1800,3200].forEach(ms=>setTimeout(schedule,ms));
  /* interval removed — bindCapture has one-time guard */
})();

/* ═══ DVL_BETA_0334_CG_GEAR_HARD_FIX_SCRIPT (L57774-57940) ═══ */
(function(){
  'use strict';

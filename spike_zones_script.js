<!-- ── DVL Spike Zones Engine (Beta 0.160) ── -->
...
(function DVL_SPIKE_ZONES(){
  ...
  function _cfg(){
    return{
      minLevel:  clp(parseInt(sv('szMinLevel','3'))||3,1,10),
      ...
      nested:    sv('szNested',''),
      nestMinLvl:clp(parseInt(sv('szNestMinLvl','3'))||3,1,10),
  ...
  function drawSpikeZones(ctx,W,H,V,sc,x,bw){
    ...
    var cols=[null];
    for(var _li=1;_li<=10;_li++){var _hx=cfg['colorL'+_li];cols.push(_hx?hexToRgb(_hx)||COL[_li]:COL[_li]);}
    ...
    for(var zi=0;zi<main.length;zi++){
      ...
      /* borders */
      ctx.lineWidth=z.level>=8?2.5:z.level>=5?1.5:1;
